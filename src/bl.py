...
class Bottom_Left():
    def __init__(self, W, L, R, C, seq, NFP, DIFP, T=None, debug=False):
        ...
        # 🔴 OTIMIZAÇÃO: PRÉ-CALCULA TODOS OS POLÍGONOS DAS PEÇAS
        self.poligonos_pecas = [Polygon(pol) for pol in T]
    ...
    # ... (mantém os métodos de plotagem iguais) ...
    ...
    def _plot_peca(self, peca_tipo, posicao, cor='green', alpha=0.5):
        ...
        poligono = self.poligonos_pecas[peca_tipo]  # 🔴 USA O PRÉ-CALCULADO
    ...
    def _plot_estado_atual(self, t, pecas_posicionadas, S_array, pontos_antes_nfp=None, 
        ...
        if len(S_array) > 0:
            poligono_peca = self.poligonos_pecas[t]  # 🔴 USA O PRÉ-CALCULADO
    ...
    def rodar(self, verbose=False):
        ...
        for t in self.seq_demanda:
            ...
            # 🔴 OTIMIZAÇÃO: Aplica restrições dos NFPs
            for tipo_u, pos_u in pecas_posicionadas:
                ...
                # 🔴 OTIMIZAÇÃO: Filtro bounding box (rápido)
                ...
                if len(pontos_bbox) > 0:
                    # 🔴 OTIMIZAÇÃO: List comprehension (mais rápida que loop manual)
                    # e verifica se o ponto está dentro do NFP
                    pontos_trans = [(p[0] - pos_u[0], p[1] - pos_u[1]) for p in pontos_bbox]
                    
                    # 🔴 OTIMIZAÇÃO: Usa list comprehension com covers
                    dentro = [nfp_prep.covers(Point(p)) for p in pontos_trans]
                    
                    # 🔴 OTIMIZAÇÃO: Remove pontos em uma única operação
                    if any(dentro):
                        manter_mask = np.ones(len(S_array), dtype=bool)
                        manter_mask[bbox_mask] = ~np.array(dentro)
                        S_array = S_array[manter_mask]
            ...
            # 🔴 OTIMIZAÇÃO: Ordenação left-bottom
            indices_ordenados = np.lexsort((S_array[:, 1], S_array[:, 0]))
