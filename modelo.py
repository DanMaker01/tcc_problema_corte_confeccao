# ---------------------------------------------------------------------------
# Modelo heurístico para o Problema de Corte de Tecido 
# ---------------------------------------------------------------------------
...
class Modelo():
    ...
    def _plotar_resultado(self, malha, pontos, sequencia, titulo="Resultado do Posicionamento", 
        ...
        try:
            ...
            area_utilizada = self._calcular_area_utilizada_sequencia(sequencia)
    ...
    def _calcular_area_utilizada_sequencia(self, sequencia):
        """
        ...
        for tipo in sequencia:
            if 0 <= tipo < len(self.T):
                poligono_original = Polygon(self.T[tipo])
    ...
    def _salvar_todos_DIFP(self,T,malha, DIFP):
        ...
        for t in range(len(T)):
            ...
            nome_arquivo = f"{str_malha}/{str_t}.difp"
    ...
    def _rodar_BL(self,seq) -> float:   ### implementar
        ...
        if largura_ja_existente != None:
            ...
            bl_resultado = bl.rodar()           # list[Tuple[int, Tuple[float,float]]]
            # bl_resultado_pontos = [bl_resultado[i][1] for i in range(len(bl_resultado))]
            # self._plotar_resultado(M,bl_resultado_pontos,seq)
            largura_resultado = self._medir_largura_faixa_BL(bl_resultado)
            print("largura =",largura_resultado)
